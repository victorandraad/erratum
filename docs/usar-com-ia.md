...
O `erratum` só rende se o agente chamar nos três momentos certos. Ele não adivinha: alguém precisa
...
Códigos de saída: 0 sempre, menos `gate` reprovado (1) e uso errado (2). Dá para usar o `gate`
...
## 5. Plugar uma memória externa (`ERRATUM_SEARCH_CMD`)
